...
uint8_t ErrorRecovery::getHealthScore() {
    ...
    if (guard) {
        ...
        for (const auto& record : errorHistory_) {
            if (!record.recovered && (now - record.lastOccurrence) < SystemConstants::ErrorLogging::ERROR_WINDOW_MS) {
...
uint32_t ErrorRecovery::calculateBackoff(uint32_t baseDelay, uint32_t retryCount, float multiplier) {
    ...
    for (uint32_t i = 0; i < retryCount; i++) {
        ...
        // Cap at 5 minutes
        if (delay > SystemConstants::ErrorLogging::RATE_LIMIT_MAX_INTERVAL_MS) {
            delay = SystemConstants::ErrorLogging::RATE_LIMIT_MAX_INTERVAL_MS;
            break;
        }
    }
    return delay;
