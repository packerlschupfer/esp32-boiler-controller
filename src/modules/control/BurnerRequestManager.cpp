...
bool BurnerRequestManager::atomicUpdateBits(EventBits_t setBits, EventBits_t clearBits) {
    ...
    if (changed) {
        ...
        const EventBits_t STATE_BITS = SystemEvents::BurnerRequest::HEATING |
                                       ...
                                       SystemEvents::BurnerRequest::PRIORITY_WATER |
                                       SystemEvents::BurnerRequest::POWER_LOW |
                                       SystemEvents::BurnerRequest::POWER_HIGH |
                                       SystemEvents::BurnerRequest::TEMPERATURE_MASK;

        if (changeEvents != 0) {
            LOG_INFO(TAG, "Setting change event bits: 0x%06X", changeEvents);
        }

        // Clear only state bits (not change events), then set new state and change events
...
bool BurnerRequestManager::setHeatingRequest(Temperature_t targetTemp, bool highPower) {
    ...
    EventBits_t clearBits = SystemEvents::BurnerRequest::WATER | SystemEvents::BurnerRequest::PRIORITY_WATER |
...
bool BurnerRequestManager::setWaterRequest(Temperature_t targetTemp, bool highPower, bool priority) {
    // Validate temperature (30°C to 65°C)
    Temperature_t minTemp = tempFromWhole(30);
    Temperature_t maxTemp = tempFromWhole(65);
    if (targetTemp < minTemp) targetTemp = minTemp;
    if (targetTemp > maxTemp) targetTemp = maxTemp;
    ...
    if (priority) {
        setBits |= SystemEvents::BurnerRequest::PRIORITY_WATER;
    }

    // Clear heating bit and old temperature if priority is set
...
bool BurnerRequestManager::clearRequest(RequestSource source) {
    ...
    switch (source) {
        ...
        case RequestSource::WATER:
            clearBits = SystemEvents::BurnerRequest::WATER | SystemEvents::BurnerRequest::PRIORITY_WATER;
...
Temperature_t BurnerRequestManager::getCurrentTargetTemp() {
    ...
    return (Temperature_t)SystemEvents::BurnerRequest::decode_temperature(bits);
}
...
bool BurnerRequestManager::isWaterPriority() {
    return (getCurrentRequests() & SystemEvents::BurnerRequest::PRIORITY_WATER) != 0;
}
