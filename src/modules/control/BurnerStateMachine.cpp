...
BurnerSMState BurnerStateMachine::handleRunningLowState() {
    // M1: Use common helper for mode switch detection
...
void BurnerStateMachine::onEnterIgnition() {
    ...
    // Determine requested power level from burner request bits
    EventBits_t requestBits = xEventGroupGetBits(SRP::getBurnerRequestEventGroup());
    bool highPowerRequested = (requestBits & SystemEvents::BurnerRequest::POWER_HIGH) != 0;
    PowerLevel startPower = highPowerRequested ? PowerLevel::FULL : PowerLevel::HALF;
    LOG_INFO(TAG, "Starting with %s power (from request)", highPowerRequested ? "FULL" : "HALF");
...
void BurnerStateMachine::onEnterRunningLow() {
    ...
    if (controller) {
        ...
        if (result.isError()) {
            LOG_ERROR(TAG, "RUNNING_LOW: Failed to set power level: %s", result.message().c_str());
            // Continue - burner is still running, just at potentially wrong power level
        }
    }
...
void BurnerStateMachine::onEnterRunningHigh() {
    ...
    if (controller) {
        ...
        if (result.isError()) {
            LOG_ERROR(TAG, "RUNNING_HIGH: Failed to set power level: %s", result.message().c_str());
            // Continue - burner is still running, just at potentially wrong power level
        }
    }
    // Set system burner on bit
...
BurnerSMState BurnerStateMachine::handleModeSwitchingState() {
    ...
    if (!newModeHasDemand) {
        // No demand in new mode - immediately go to POST_PURGE
        // Event-driven design: don't block waiting for other tasks
        // HeatingControlTask will set HEATING request when it needs the burner
        LOG_WARN(TAG, "New mode has no heat demand - transitioning to POST_PURGE");
        return BurnerSMState::POST_PURGE;
    }
...
void BurnerStateMachine::logStateTransition(BurnerSMState from, BurnerSMState to) {
    ...
    // Skip MODE_SWITCHING transitions - power level doesn't actually change during mode switch
