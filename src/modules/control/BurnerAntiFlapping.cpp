...
bool BurnerAntiFlapping::canTurnOn() {
    ...
    if (!allowed) {
        uint32_t remaining = SystemConstants::Burner::MIN_OFF_TIME_MS - elapsedOff;
...
bool BurnerAntiFlapping::canTurnOff() {
    ...
    if (!allowed) {
        uint32_t remaining = SystemConstants::Burner::MIN_ON_TIME_MS - elapsedOn;
...
bool BurnerAntiFlapping::canChangePowerLevel(PowerLevel newLevel) {
    ...
    if (!allowed) {
        uint32_t remaining = SystemConstants::Burner::MIN_POWER_CHANGE_INTERVAL_MS - elapsedSinceChange;
...
BurnerAntiFlapping::PowerLevel BurnerAntiFlapping::stateToPowerLevel(BurnerSMState state) {
    switch (state) {
        ...
        case BurnerSMState::IGNITION:
            // During ignition, burner starts at FULL power
            return PowerLevel::POWER_HIGH;
