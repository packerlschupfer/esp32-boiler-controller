...
// Pump motor protection - check if 30s has elapsed since last state change
...
bool RelayControlTask::checkPumpProtection(uint8_t relayIndex, bool desiredState) {
    ...
    if (elapsedMs < SystemConstants::Timing::PUMP_MIN_STATE_CHANGE_MS) {
        // Block the state change - motor protection
        uint32_t remainingMs = SystemConstants::Timing::PUMP_MIN_STATE_CHANGE_MS - elapsedMs;
...
uint32_t RelayControlTask::getPumpProtectionTimeRemaining(uint8_t relayIndex) {
    ...
    return SystemConstants::Timing::PUMP_MIN_STATE_CHANGE_MS - elapsedMs;
}
...
void RelayControlTask::checkRelayHealthAndEscalate(uint8_t relayIndex, bool success) {
    ...
    if (success) {
        ...
        if (consecutiveFailures[idx] >= MAX_CONSECUTIVE_FAILURES) {
            ...
            // Escalate to CentralizedFailsafe with WARNING level
            // (CRITICAL would trigger emergency shutdown, WARNING triggers monitoring)
            CentralizedFailsafe::triggerFailsafe(
