...
#include <esp_task_wdt.h>  // For unsubscribing from global watchdog
...
static bool performNTPSync() {
    ...
    if (result.success) {
        ...
        if (rtcUpdateCallback) {
            ...
            struct tm* utm = gmtime_r(&utcTime, &utmBuf);
            struct tm* ltm = localtime_r(&utcTime, &ltmBuf);  // Use utcTime with localtime_r to get system TZ applied
...
void NTPTaskEventDriven(void* parameter) {
    ...
    // Arduino framework auto-subscribes all tasks to ESP32 global watchdog
    // Log status for diagnostics (no need to store - it's always subscribed)
    esp_err_t wdt_status = esp_task_wdt_status(NULL);
    if (wdt_status == ESP_OK) {
        LOG_INFO(TAG, "ESP32 global watchdog active - feeding both ESP32 and TaskManager");
    } else {
        LOG_WARN(TAG, "ESP32 global watchdog not active - unusual, feeding TaskManager only");
    }
    ...
    if (!SRP::getTaskManager().registerCurrentTaskWithWatchdog("NTPTask", wdtConfig)) {
        ...
        LOG_INFO(TAG, "TaskManager WDT registered with 60s timeout");
    }
    ...
    while (true) {
        // Wait for events with timeout for watchdog feeding
        ...
        // Feed both watchdogs (ESP32 global and TaskManager)
        esp_task_wdt_reset();
        ...
        if (millis() - lastNetworkCheck > 10000) {  // Every 10 seconds
            lastNetworkCheck = millis();
            if (!initialSyncDone && EthernetManager::isConnected()) {
        ...
        if (events & NTP_EVENT_SYNC_NOW) {
            if (xSemaphoreTake(ntpMutex, pdMS_TO_TICKS(100)) == pdTRUE) {
                ...
                // Feed both watchdogs after NTP sync (which can take up to 5 seconds)
                esp_task_wdt_reset();
                (void)SRP::getTaskManager().feedWatchdog();
                ...
                if (success) {
                    ...
                    if (syncFailureCount >= RTC_FALLBACK_THRESHOLD && !rtcFallbackUsed && !initialSyncDone) {
                        ...
                        // Feed both watchdogs after RTC fallback attempt
                        esp_task_wdt_reset();
                        (void)SRP::getTaskManager().feedWatchdog();
                    }
