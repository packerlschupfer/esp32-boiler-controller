...
static bool tryRTCFallback() {
    ...
    // Note: RTC only has 1-second resolution, so tv_usec will be 0
    struct timeval tv = { .tv_sec = rtcTime, .tv_usec = 0 };
    settimeofday(&tv, nullptr);

    LOG_INFO(TAG, "System time set from RTC fallback: %04d-%02d-%02d %02d:%02d:%02d (sub-second precision: 0)",
        ...
        if (driftTracker.lastSyncTime > 0 && driftTracker.lastSyncMillis > 0) {
            ...
            if (millisElapsed > 60000) {  // Only calculate drift after at least 1 minute
                ...
                if (timeDiffSeconds > 3600 || timeDiffSeconds < -3600) {
                    ...
                    // The offset is what NTP corrected, which artificially inflates drift
                    driftMs -= result.offsetMs;
                }
                ...
                // Optional: Display sub-millisecond precision when NTP library is updated
                // LOG_INFO(TAG, "Time synced from %s, offset %ldms.%03lu, RTT %dms",
                //          result.serverUsed, result.offsetMs, result.syncUsec/1000, result.roundTripMs);
            ...
            // Optional: Display sub-millisecond precision when NTP library is updated
            // LOG_INFO(TAG, "Initial time sync from %s, offset %ldms.%03lu, RTT %dms",
            //          result.serverUsed, result.offsetMs, result.syncUsec/1000, result.roundTripMs);
