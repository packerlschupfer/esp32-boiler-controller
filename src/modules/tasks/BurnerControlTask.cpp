...
void BurnerControlTask(void* parameter) {
    ...
    EventBits_t initialSystemStateBits = xEventGroupGetBits(cachedHandles.systemStateEventGroup);
    if ((initialSystemStateBits & SystemEvents::SystemState::HEATING_ENABLED) && 
        (initialSystemStateBits & SystemEvents::SystemState::WATER_ENABLED)) {
        TemperatureSensorFallback::setOperationMode(
            TemperatureSensorFallback::OperationMode::BOTH);
        LOG_INFO(TAG, "Initial operation mode set to BOTH");
    } else if (initialSystemStateBits & SystemEvents::SystemState::HEATING_ENABLED) {
        TemperatureSensorFallback::setOperationMode(
            TemperatureSensorFallback::OperationMode::SPACE_HEATING);
        LOG_INFO(TAG, "Initial operation mode set to SPACE_HEATING");
    } else if (initialSystemStateBits & SystemEvents::SystemState::WATER_ENABLED) {
        TemperatureSensorFallback::setOperationMode(
            TemperatureSensorFallback::OperationMode::WATER_HEATING);
        LOG_INFO(TAG, "Initial operation mode set to WATER_HEATING");
    } else {
        TemperatureSensorFallback::setOperationMode(
            TemperatureSensorFallback::OperationMode::NONE);
        LOG_INFO(TAG, "Initial operation mode set to NONE");
    }
    ...
    while (true) {
        ...
        static uint32_t emergencyStopTime = 0;
        ...
        if ((emergencyBits & SystemEvents::SystemState::EMERGENCY_STOP) && !emergencyStopActive) {
            emergencyStopActive = true;
            emergencyStopTime = millis();
            LOG_ERROR(TAG, "Emergency stop initiated");
            BurnerStateMachine::emergencyStop();
        } else if (emergencyStopActive && Utils::elapsedMs(emergencyStopTime) > EMERGENCY_COOLDOWN_MS) {
        ...
        if (requestEvents & SystemEvents::BurnerRequest::CHANGE_EVENT_BITS) {
            LOG_INFO(TAG, "Processing burner request change event (events: 0x%06X)", requestEvents);
        ...
        // 6. Check pump cooldown (pumps continue after burner stops for heat dissipation)
        BurnerSystemController* controller = SRP::getBurnerSystemController();
        if (controller && controller->checkAndHandleCooldown()) {
            LOG_INFO(TAG, "Pump cooldown complete - pumps turned off");
        }
...
static void processBurnerRequest() {
    ...
    if (!(systemStateBits & SystemEvents::SystemState::BOILER_ENABLED)) {
        ...
        // Clear burner state bits
        xEventGroupClearBits(cachedHandles.systemStateEventGroup,
            SystemEvents::SystemState::BURNER_OFF | SystemEvents::SystemState::BURNER_HEATING_LOW | SystemEvents::SystemState::BURNER_HEATING_HIGH |
            SystemEvents::SystemState::BURNER_WATER_LOW | SystemEvents::SystemState::BURNER_WATER_HIGH);
    ...
    bool waterPriority = (requestBits & SystemEvents::BurnerRequest::PRIORITY_WATER) != 0;
...
static void updateBurnerState(bool heatDemand, bool isWaterMode, Temperature_t targetTemp, bool highPower) {
    ...
    // Clear previous burner state bits
    xEventGroupClearBits(cachedHandles.systemStateEventGroup,
        SystemEvents::SystemState::BURNER_OFF | SystemEvents::SystemState::BURNER_HEATING_LOW | SystemEvents::SystemState::BURNER_HEATING_HIGH |
        SystemEvents::SystemState::BURNER_WATER_LOW | SystemEvents::SystemState::BURNER_WATER_HIGH);
    ...
    if (heatDemand) {
        ...
        if (heatDemand) {
            ...
            if (validationResult != BurnerSafetyValidator::ValidationResult::SAFE_TO_OPERATE) {
                ...
                if (validationResult != BurnerSafetyValidator::ValidationResult::SENSOR_FAILURE &&
    ...
    switch (currentState) {
        ...
        case BurnerSMState::RUNNING_LOW:
            if (isWaterMode) {
                xEventGroupSetBits(cachedHandles.systemStateEventGroup, SystemEvents::SystemState::BURNER_WATER_LOW);
            } else {
                xEventGroupSetBits(cachedHandles.systemStateEventGroup, SystemEvents::SystemState::BURNER_HEATING_LOW);
            }
            break;
        case BurnerSMState::RUNNING_HIGH:
            if (isWaterMode) {
                xEventGroupSetBits(cachedHandles.systemStateEventGroup, SystemEvents::SystemState::BURNER_WATER_HIGH);
            } else {
                xEventGroupSetBits(cachedHandles.systemStateEventGroup, SystemEvents::SystemState::BURNER_HEATING_HIGH);
            }
            break;
        ...
        case BurnerSMState::ERROR:
            if (cachedHandles.initialized) {
                ...
                if (systemEventGroup) {
                    xEventGroupSetBits(systemEventGroup, SystemEvents::SystemState::BURNER_ERROR);
                }
            }
            break;
        default:
