...
static void handleReadTick(RYN4* ryn4) {
    ...
    if (actual != desired) {
        ...
        uint8_t mismatchMask = actual ^ desired;  // XOR to find differing bits
        uint8_t delayMask = g_relayState.delayMask.load(std::memory_order_acquire);
        uint8_t realMismatch = mismatchMask & ~delayMask;  // Exclude DELAY relays

        if (realMismatch == 0) {
            // All mismatches are from active DELAY commands - expected!
            LOG_DEBUG(TAG, "Relay verification deferred (DELAY active): Desired: 0x%02X, Actual: 0x%02X, Delay mask: 0x%02X",
                      desired, actual, delayMask);
        ...
        if (mismatches == 1) {
            ...
            LOG_DEBUG(TAG, "Relay verification pending (attempt 1/2): Desired: 0x%02X, Actual: 0x%02X",
                      desired, actual);
        } else {
            // Persistent mismatch - real problem!
            LOG_ERROR(TAG, "Relay verification FAILED after %d attempts! Desired: 0x%02X, Actual: 0x%02X",
                      mismatches, desired, actual);

            // Log individual mismatches (excluding DELAY relays)
            for (int i = 0; i < 8; i++) {
                if (realMismatch & (1 << i)) {
                    bool desiredBit = (desired >> i) & 0x01;
                    bool actualBit = (actual >> i) & 0x01;
                    LOG_ERROR(TAG, "  Relay %d: desired=%s, actual=%s",
                              i + 1, desiredBit ? "ON" : "OFF", actualBit ? "ON" : "OFF");
                }
            }
