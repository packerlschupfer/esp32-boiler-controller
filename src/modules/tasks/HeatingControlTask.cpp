    ...
    switch (heatingState.state) {
        case HeatingOff: {
            ...
            if (checkIfSpaceHeatingNeededEvent() || (controlBits & SystemEvents::ControlRequest::HEATING_ON_OVERRIDE)) {
                ...
                if (waterPriorityEnabled && waterEnabled) {
                    // Wait briefly for water priority to be claimed OR released
                    // This is event-based: either WATER request bit is set (water claimed priority)
                    // or WATER_PRIORITY_RELEASED is set (water doesn't need priority)
                    EventBits_t waitBits = xEventGroupWaitBits(
                        SRP::getBurnerRequestEventGroup(),
                        SystemEvents::BurnerRequest::WATER | SystemEvents::BurnerRequest::PRIORITY_WATER,
                        pdFALSE,  // Don't clear bits
                        pdFALSE,  // Wait for any bit
                        pdMS_TO_TICKS(200)  // 200ms timeout - if neither set, water doesn't need burner
                    );
                    (void)waitBits;  // We check the result below
                }

                // Check if water heating has priority
                ...
                bool waterPriority = (burnerRequestBits & SystemEvents::BurnerRequest::PRIORITY_WATER) != 0;
                ...
                if (heatingState.heatingControl != nullptr) {
                    ...
                    return;  // Don't continue if module unavailable
                }
        ...
        case HeatingOn: {
            ...
            if ((controlBits & SystemEvents::ControlRequest::HEATING_OFF_OVERRIDE) ||
                ...
                uint32_t now = millis();
                bool targetChanged = (abs(newBoilerTarget - heatingState.lastBoilerTarget) > 10);  // 1°C in 0.1°C units
