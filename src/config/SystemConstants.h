...
namespace SystemConstants {
    ...
    namespace Timing {
        ...
        constexpr uint32_t PUMP_COOLDOWN_MS = 120000;           // 2 min - pump continues after burner stops (heat dissipation)
    ...
    namespace Temperature {
        ...
        constexpr Temperature_t MAX_BOILER_TEMP_C = 900;        // 90.0°C
        constexpr Temperature_t MIN_BOILER_TEMP_C = 200;        // 20.0°C
        constexpr Temperature_t CRITICAL_BOILER_TEMP_C = 950;   // 95.0°C
        constexpr Temperature_t FREEZE_PROTECTION_TEMP_C = 50;  // 5.0°C
        ...
        // Auto-tuning temperature constants
        constexpr Temperature_t AUTOTUNE_TEMP_INCREMENT_C = 100;   // 10.0°C above current
        constexpr Temperature_t AUTOTUNE_MAX_SETPOINT_C = 300;     // 30.0°C safety limit
    }
    ...
    namespace PID {
        ...
        constexpr float LEVEL_3_THRESHOLD = 5.0f;    // Maintain
        constexpr float LEVEL_4_THRESHOLD = 20.0f;   // Light heating
        constexpr float LEVEL_5_THRESHOLD = 50.0f;   // Moderate heating
        // Above LEVEL_5_THRESHOLD = Maximum heating
        ...
        namespace Autotune {
            constexpr uint8_t MIN_CYCLES = 3;              // Minimum oscillation cycles needed
            constexpr uint8_t MAX_CYCLES = 10;             // Maximum cycles before timeout
            constexpr float MAX_TUNING_TIME_SECONDS = 600.0f;  // 10 minutes maximum tuning time
            constexpr float DEFAULT_RELAY_AMPLITUDE = 40.0f;    // Default relay output amplitude
            constexpr float DEFAULT_RELAY_HYSTERESIS = 1.0f;    // Default hysteresis band
    ...
    namespace Communication {
        ...
        constexpr uint32_t MODBUS_RETRY_DELAY_MS = 100;
        constexpr uint32_t MODBUS_INTER_FRAME_DELAY_MS = 5;
        constexpr uint32_t SENSOR_RETRY_DELAY_MS = 50;     // Delay between sensor read retries
    ...
    // ===========================
    namespace Hardware {
        // Relay function assignments (1-based indexing)
        namespace RelayFunction {
            constexpr uint8_t HEATING_PUMP = 1;      // Heating circulation pump
            constexpr uint8_t WATER_PUMP = 2;        // Water heating circulation pump
            constexpr uint8_t BURNER_ENABLE = 3;     // Main burner on/off control
            constexpr uint8_t POWER_SELECT = 4;      // Power level selector (ON=half, OFF=full)
            constexpr uint8_t WATER_MODE = 5;        // Water heating mode
            constexpr uint8_t VALVE = 6;             // Valve control
            constexpr uint8_t SPARE_1 = 7;           // Spare relay 1
            constexpr uint8_t ALARM = 8;             // Alarm/buzzer relay
            
            constexpr uint8_t RELAY_COUNT = 8;       // Total number of relays
        }
