...
void setup() {
    ...
    // Send boot announcement to syslog (bypasses minLevel filter)
    if (auto* syslog = SRP::getSyslog()) {
        syslog->sendUnfiltered(ESP_LOG_INFO, TAG, "Ready");
    }
...
void handleRuntimeTasks() {
    ...
    if (gSystemInitializer && gSystemInitializer->isFullyInitialized()) {
        ...
        if (now - lastMemoryReport > SystemConstants::System::MEMORY_REPORT_INTERVAL_MS) {
