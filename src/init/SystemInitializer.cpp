...
void SystemInitializer::cleanupControlModules() {
    ...
    // Note: PumpControlModule uses static methods, no cleanup needed
...
Result<void> SystemInitializer::initializeControlModules() {
    ...
    // Note: PumpControlModule uses static methods - no allocation needed
    // Tasks are created directly by TaskInitializer using PumpControlModule::HeatingPumpTask
...
Result<void> SystemInitializer::initializeMQTT() {
    ...
    // Build MQTT URI and last will topic
    char mqttUri[128];
    snprintf(mqttUri, sizeof(mqttUri), "mqtt://%s:%d", MQTT_SERVER, MQTT_PORT);
    char willTopic[64];
    snprintf(willTopic, sizeof(willTopic), "%s/status", MQTT_CLIENT_ID);

    // Initialize MQTT connection using builder pattern
    auto config = MQTTConfig(mqttUri)
        .withClientId(MQTT_CLIENT_ID)
        .withCredentials(MQTT_USERNAME, MQTT_PASSWORD)
        .withKeepAlive(MQTT_KEEP_ALIVE_SECONDS)
        .withLastWill(willTopic, "offline", 1, true)
        .withAutoReconnect(true);

    auto result = mqttManager_->begin(config);
    if (!result.isOk()) {
        mqttManager_ = nullptr;
        return Result<void>(SystemError::MQTT_CONNECT_FAILED, "Failed to initialize MQTT connection");
    }
