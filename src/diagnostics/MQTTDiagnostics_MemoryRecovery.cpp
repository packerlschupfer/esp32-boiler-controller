...
size_t MQTTDiagnostics::clearDiagnosticsBuffers() {
    ...
    if (diag->taskHandle != nullptr) {
        ...
        if (taskState != eSuspended) {
            ...
            scheduleDiagnosticsRecovery(30000);
...
void MQTTDiagnostics::restoreNormalOperation() {
    ...
    diag->intervals.memory = 300000;       // 5 minutes
    diag->intervals.tasks = 300000;        // 5 minutes
    diag->intervals.sensors = 30000;       // 30 seconds
    diag->intervals.relays = 10000;        // 10 seconds
    diag->intervals.network = 60000;       // 1 minute
    diag->intervals.performance = 60000;   // 1 minute
    diag->intervals.pid = 5000;           // 5 seconds
    diag->intervals.burner = 5000;        // 5 seconds
    diag->intervals.maintenance = 3600000; // 1 hour
