...
ITemperatureSensor::Reading ANDRTF3TemperatureSensor::readTemperature(uint8_t channel) {
    ...
    if (requestAsyncReading()) {
        if (processAsyncReading(100)) {  // Wait up to 100ms
        ...
        // Fall back to synchronous reading with retry
        LOG_DEBUG(TAG, "Falling back to synchronous read");

        // Try up to 3 times with a small delay between attempts
        const int MAX_RETRIES = 3;
        bool readSuccess = false;

        for (int retry = 0; retry < MAX_RETRIES && !readSuccess; retry++) {
            // Feed watchdog before each Modbus operation - these can block for
            // up to 3 seconds if waiting for bus mutex or response timeout
            (void)SRP::getTaskManager().feedWatchdog();

            if (retry > 0) {
                vTaskDelay(pdMS_TO_TICKS(SystemConstants::Communication::SENSOR_RETRY_DELAY_MS));
                LOG_DEBUG(TAG, "Retry %d/%d for synchronous read", retry, MAX_RETRIES - 1);
            }

            if (device->readTemperature()) {
                int16_t tempValue = device->getTemperature();  // Returns value * 10

                // Validate temperature range (-40°C to +125°C)
                if (tempValue >= -400 && tempValue <= 1250) {
                    result.temperature = tempValue / 10.0f;
                    result.valid = true;
                    result.timestamp = millis();
                    readSuccess = true;

                    LOG_DEBUG(TAG, "Temperature (sync): %.1f°C", result.temperature);
                } else {
                    LOG_WARN(TAG, "Temperature out of range: %d", tempValue);
                }
            }
        }

        if (!readSuccess) {
            LOG_WARN(TAG, "Synchronous read failed after %d attempts", MAX_RETRIES);
        }
    }
