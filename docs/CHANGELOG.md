...
- Finding, not changed: no task sets the per-channel `SensorUpdate` bits (`BOILER_OUTPUT`, `BOILER_RETURN`, `WATER_TANK`, `OUTSIDE`) in SRP's sensor event group. BoilerTempControlTask's wait on `BOILER_OUTPUT` always ends in its 5 s timeout, BurnerControlTask's temperature trigger (with its sensor failure check) never fires, and `DATA_AVAILABLE` is never cleared after the first read
