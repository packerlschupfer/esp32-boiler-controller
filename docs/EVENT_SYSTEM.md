...
**Consumers**: BoilerTempControlTask waits on `BOILER_OUTPUT` (clear-on-exit, 5 s timeout while the boiler output reading is invalid), so its PID runs once per read (~2.5 s); BurnerControlTask polls `BOILER_RETURN | WATER_TANK` and the `BOILER_OUTPUT/BOILER_RETURN/WATER_TANK_ERROR` bits (one wake-up per read) and runs the state machine update and the sensor check (`SensorFailureConfirm`: emergency stop after 10 s of missing sensors with heat demand); SafetyInterlocks reads `DATA_AVAILABLE` (set after the first read, never cleared); MonitoringTask and OtaRollbackGuard read `FIRST_READ_COMPLETE`.
