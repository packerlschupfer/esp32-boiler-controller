...
**Who sets bits in this group**: MB8ARTTasks, after every MB8ART read, sets each active channel's update bit (valid reading) or `*_ERROR` bit (invalid) from `SensorHardware::CONFIGS` (index = channel from `SensorIndices.h`: CH0 `BOILER_OUTPUT`, CH1 `BOILER_RETURN`, CH2 `WATER_TANK`, CH3 `OUTSIDE`; CH4 pressure has no channel bits), plus `DATA_AVAILABLE`, `DATA_ERROR`, `FIRST_READ_COMPLETE`, `PRESSURE` and `PRESSURE_ERROR`. ANDRTF3Task sets `INSIDE` and `INSIDE_ERROR`. The MB8ART library keeps its own copy of the channel bits in a separate event group (`MB8ART::getSensorEventGroup()`), polled by MB8ARTTasks. Before 2026-09-15 the channel bits were never set in this group.

**Consumers**: BoilerTempControlTask waits on `BOILER_OUTPUT` (clear-on-exit, 5 s timeout while the boiler output reading is invalid), so its PID runs once per read (~2.5 s); BurnerControlTask polls `BOILER_RETURN | WATER_TANK` and the `BOILER_OUTPUT/BOILER_RETURN/WATER_TANK_ERROR` bits, but only when its own request-bit wait ends (100 ms while running, 1 s, 3 s when idle), so it can react up to 3 s late in idle. It then runs the state machine update, and the sensor check only while a mode request is active or the demand is armed (`SensorFailureConfirm`: emergency stop after 10 s of missing sensors with demand; a missing boiler output reading trips the 5 s full safety check earlier, see SAFETY_SYSTEM.md); SafetyInterlocks reads `DATA_AVAILABLE` (set after the first read, never cleared; a lost sensor link is caught by the stale-sensor emergency stop instead); MonitoringTask and OtaRollbackGuard read `FIRST_READ_COMPLETE`.
