...
public:
    ...
    void publishStatus() {
        ...
        std::string state = settings_->heatingEnabled ? "heating" : "idle";
        client_->publish(baseTopic_ + "/state/system", state, 0, true);

        // Publish temperatures (using correct field names)
        char buffer[256];
        snprintf(buffer, sizeof(buffer),
                "{\"boiler_out\":%.1f,\"boiler_return\":%.1f,\"water\":%.1f,\"inside\":%.1f}",
                readings_->boilerTempOutput / 10.0f,
                readings_->boilerTempReturn / 10.0f,
                readings_->waterHeaterTempTank / 10.0f,
...
private:
    void handleEnableCommand(const std::string& payload) {
        if (payload == "on") {
            ...
            settings_->heatingEnabled = false;
    ...
    void handleHeatingCommand(const std::string& payload) {
        if (payload == "on") {
            requestManager_->requestHeating(settings_->targetTemperatureInside, 100);
        } else if (payload == "off") {
    ...
    void handleParameterSet(const std::string& topic, const std::string& payload) {
        ...
        if (param == "heating_target") {
            settings_->targetTemperatureInside = tempFromFloat(value);
...
void test_status_publishing() {
    ...
    // Set up test data (using correct field names)
    readings->boilerTempOutput = tempFromFloat(65.5f);
    ...
    TEST_ASSERT(messages[1].payload.find("45.0") != std::string::npos);
...
void test_remote_control() {
    ...
    TEST_ASSERT_FALSE(settings->heatingEnabled);
    
    mqttClient->simulateIncomingMessage("esplan/boiler/control/enable", "on");
    TEST_ASSERT_TRUE(settings->heatingEnabled);
...
void test_parameter_updates() {
    ...
    TEST_ASSERT_EQUAL_INT16(235, settings->targetTemperatureInside);  // 23.5 * 10
