...
public:
    ...
    FallbackMode evaluateMode(const SharedSensorReadings& readings) {
        ...
        if (readings.boilerTempInput == TEMP_INVALID) invalidCount++;
        if (readings.boilerTempOutput == TEMP_INVALID) invalidCount++;
        if (readings.waterTemp == TEMP_INVALID) invalidCount++;
        if (readings.returnTemp == TEMP_INVALID) invalidCount++;
