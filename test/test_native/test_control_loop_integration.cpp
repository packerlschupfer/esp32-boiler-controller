...
public:
    ...
    MockSystemComponents() {
        // Initialize with default settings (using correct field names)
        settings.targetTemperatureInside = tempFromFloat(21.0f);
        settings.heating_hysteresis = tempFromFloat(2.0f);
        settings.useWeatherCompensatedControl = false;
        ...
        // Initialize readings with safe values (using correct field names)
        readings.boilerTempOutput = tempFromFloat(50.0f);
        readings.boilerTempReturn = tempFromFloat(40.0f);
    ...
    void simulateTemperatureChange(float deltaTemp) {
        ...
        readings.boilerTempReturn = readings.boilerTempReturn + tempFromFloat(deltaTemp * 1.5);
    }
};
...
void test_basic_heating_control_loop() {
    ...
    system.settings.targetTemperatureInside = tempFromFloat(21.0f);
...
void test_water_heating_priority() {
    ...
    system.readings.insideTemp = tempFromFloat(18.0f);
    system.readings.waterHeaterTempTank = tempFromFloat(40.0f);
    system.settings.wheaterPriorityEnabled = true;
...
void test_pid_control_response() {
    ...
    system.settings.spaceHeatingKp = 2.0f;
    system.settings.spaceHeatingKi = 0.1f;
    system.settings.spaceHeatingKd = 0.5f;
    system.settings.useBoilerTempPID = true;
...
void test_hysteresis_control() {
    ...
    bool shouldHeat1 = system.heatingControl->checkHeatingConditions(
        system.readings, system.settings.targetTemperatureInside, 
...
void test_weather_compensation() {
    ...
    system.settings.useWeatherCompensatedControl = true;
...
void test_control_loop_timing() {
    ...
    for (int seconds = 0; seconds < 60; seconds++) {
        ...
        bool heatingNeeded = system.heatingControl->checkHeatingConditions(
            system.readings, 
            system.settings.targetTemperatureInside,
            system.settings.heating_hysteresis);
        
        if (heatingNeeded) {
            system.requestManager->requestHeating(
                system.settings.targetTemperatureInside, 100);
        } else {
...
void test_heating_cycle_with_hysteresis() {
    ...
    if (currentTemp < (settings.targetTemperatureInside - settings.heating_hysteresis)) {
    ...
    if (currentTemp >= settings.targetTemperatureInside) {
