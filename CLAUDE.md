...
### Control Systems
- `src/modules/control/BurnerStateMachine.cpp` - 9-state burner FSM
...
- MB8ART 8-channel temperature sensor (Modbus RTU, RX=36, TX=4, 9600 baud, 8E1)
- RYN4 8-channel relay module (Modbus RTU, 9600 baud, 8E1)
- RS485 transceiver (MAX485 or similar)

**Modbus Configuration:**
- Baud rate: 9600 (configurable in `platformio.ini` via `-D MODBUS_BAUD_RATE=9600`)
- Format: 8E1 (8 data bits, Even parity, 1 stop bit)
- Inter-frame delay: Auto-calculated (5ms at 9600 baud)
...
Default settings:
- **Device IP**: 192.168.20.40 (static) - modify in `src/config/ProjectConfig.h`
- **Gateway**: 192.168.20.1 - modify in `src/config/ProjectConfig.h`
- **MQTT Broker**: Configure in `credentials.ini`
- **OTA Port**: 3232
- **Modbus Baud Rate**: 9600 - modify in `platformio.ini` (`-D MODBUS_BAUD_RATE=...`)
