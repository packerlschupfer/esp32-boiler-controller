 ...
 *
 * PRIMARY pump management system for the boiler controller.
 * Manages pump startup/shutdown with configurable minimum state duration
 * to protect pump motors from excessive cycling.
 *
 * Design Notes:
 * - Pumps start ATOMICALLY with burner via BurnerSystemController batch commands
 * - No separate "pump must be running first" verification - relay verification
 *   confirms command succeeded, temperature sensors detect physical pump failure
 * - Emergency bypass (forceAllPumpsOn/Off) intentionally skips motor protection
 *   for safety-critical situations
 *
 * Features:
 * - Configurable motor protection (default 30s via SafetyConfig::pumpProtectionMs)
 * - Thread-safe operation with mutex protection
 * - State tracking for protection enforcement
 * - Emergency bypass for safety-critical shutdowns
 *
 * This PumpCoordinator is the only pump management system.
 */
