...
public:
     ...
     * @param targetTemp Target temperature in Temperature_t format
     * @param highPower True for high power mode
     * @param priority True if water heating should have priority
     * @return True if request was changed
     */
    static bool setWaterRequest(bool enable, Temperature_t targetTemp = 600, bool highPower = false, bool priority = true) {
        const char* TAG = "BurnerReqHelper";
        EventGroupHandle_t eventGroup = SRP::getBurnerRequestEventGroup();
        if (!eventGroup) return false;
        ...
        if (changed) {
            LOG_INFO(TAG, "Water request changed: %s, target: %.1f°C, power: %s, priority: %s",
                     enable ? "ON" : "OFF",
                     enable ? tempToFloat(targetTemp) : 0.0f,
                     enable ? (highPower ? "HIGH" : "LOW") : "N/A",
                     enable ? (priority ? "YES" : "NO") : "N/A");
            ...
            if (enable) {
                ...
                xEventGroupClearBits(eventGroup, BURNER_REQUEST_WATER_BIT | BURNER_REQUEST_PRIORITY_WATER_BIT);
