...
public:
    ...
    /**
     * @brief Seamlessly switch between HEATING ↔ WATER modes
     *
     * Optimized mode switch for running burner that keeps flame alive:
     * - Keeps burner relays active (no ignition restart)
     * - Switches pump relays atomically (single Modbus transaction)
     * - Bypasses 30s pump protection (safe because burner stays running)
     * - Updates internal mode tracking
     *
     * ONLY valid for HEATING ↔ WATER transitions while burner is running.
     * For OFF transitions, use deactivate() followed by activateXxxMode().
     *
     * Pump protection bypass rationale:
     * - Burner stays running → continuous heat dissipation → safe for pump
     * - Mode switches are rare (~few per day) vs. normal cycling (~hourly)
     * - Only one pump changes per transition (make-before-break)
     * - Minimal impact on pump lifespan
     *
     ...
     *
     * Errors:
     * - INVALID_PARAMETER: newMode is OFF or same as current mode
     * - RELAY_OPERATION_FAILED: Batch command failed
     */
